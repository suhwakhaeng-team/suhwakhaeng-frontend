...
export default function ConfidenceLearningPage() {
  ...
  return <div className="ci-shell">
    ...
    <main className="ci-main">
      <header className="ci-heading"><div><p>통계적 추정 · Case 3</p><h1>신뢰구간</h1></div><span className="ci-xp">✦ {earned ? 100 : completed * 10} XP<small>이번 체험</small></span></header>
      <div className="ci-layout">
        ...
        <section className="ci-lesson">
          ...
          {step === 8 && (finished ? <><div className="ci-concept"><strong>선수 개념을 연결해 구간을 계산하고 해석했어요.</strong><p>새 자료 문제를 통과했어요. 운영 숙련도 판정과 시간차 복습은 연결되지 않았어요.</p></div><div className="ci-completion">✦ 100 XP<small>이번 체험 완료</small></div><button className="ci-secondary" onClick={() => { setFinished(false); setTransfer([]); setMessage(''); }}>새 자료 문제 다시 풀어보기</button></> : <><p className="ci-instruction">다른 학교의 자료예요. 정규모집단에서 독립적으로 임의추출했다고 가정해요.</p><div className="ci-given"><span>표본평균 <strong>42분</strong></span><span>표본 수 <strong>36명</strong></span><span>알려진 모표준편차 <strong>6분</strong></span><span>신뢰수준 <strong>95% · z ≈ 1.96</strong></span></div><form onSubmit={event => { event.preventDefault(); const passed = checkTransfer(transfer); setFinished(passed); if (passed) setEarned(true); setMessage(passed ? '' : '양 끝값, 신뢰수준의 의미, 표본 수에 따른 폭을 다시 확인하세요.'); }}><div className="ci-number-grid">{input(0, '아래 끝값 (분)', true)}{input(1, '위 끝값 (분)', true)}</div><fieldset className="ci-question"><legend>이 95%는 무엇을 뜻하나요?</legend><div className="ci-options ci-options-column">{[['repeat', '같은 방법을 반복할 때 약 95%의 구간이 모평균을 포함해요'], ['students', '학생 95%의 시간이 이 구간에 있어요'], ['fixed', '모평균이 움직이며 이 구간에 95% 확률로 들어와요']].map(([value, label]) => <button type="button" key={value} aria-pressed={transfer[2] === value} onClick={() => updateAnswer(2, value, true)}>{label}</button>)}</div></fieldset><fieldset className="ci-question"><legend>다른 조건이 같다면, 9명일 때보다 구간의 폭은?</legend><div className="ci-options">{[['narrow', '더 좁아요'], ['wide', '더 넓어요'], ['same', '같아요']].map(([value, label]) => <button type="button" key={value} aria-pressed={transfer[3] === value} onClick={() => updateAnswer(3, value, true)}>{label}</button>)}</div></fieldset><p className="ci-note">끝값은 소수 둘째 자리까지 입력하세요.</p><button className="ci-primary">답 확인하기</button></form></>)}
          <p className="ci-message" role="status">{message}</p>
          {step < 8 && <footer className="ci-actions"><button className="ci-secondary" disabled={step === 0} onClick={() => { setStep(previous => previous - 1); setMessage(''); }}>이전</button><button className="ci-primary" disabled={!ready} onClick={() => { setCompleted(previous => Math.max(previous, step + 1)); setStep(previous => previous + 1); setMessage(''); }}>다음 단계 →</button></footer>}
