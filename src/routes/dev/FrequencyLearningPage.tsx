...
export default function FrequencyLearningPage({ onComplete }: { onComplete?: () => void }) {
  ...
  return <div className="fl-shell">
    ...
    <main className="fl-main">
      <header className="fl-heading"><div><p>자료의 정리 · 기초 코스</p><h1>도수분포표</h1></div></header>
        ...
        <section className="fl-lesson" aria-label="현재 미션">
          ...
          <form onSubmit={event => { event.preventDefault(); if (!correct) check(); }}>
            ...
            <div className="fl-feedback" aria-live="polite">{feedback === 'incorrect' && <div className="fl-retry"><strong>조금만 다시 살펴봐요.</strong><p>{step === 1 ? '20은 포함, 30은 제외예요. 선택한 카드들을 확인해보세요.' : step >= 3 ? '각 구간의 개수와 경계값을 확인해요. 마지막 도전에서는 구간과 전체 인원도 확인해야 해요.' : '표의 해당 행 또는 구간에 들어가는 자료를 다시 확인해보세요.'}</p></div>}
              {correct && <div className="fl-success"><strong>{review && hint ? '도움을 받아 해결했어요!' : review ? '새로운 자료에서도 성공했어요!' : '미션 성공!'}</strong><p>{review && hint ? '복습 확인 배지는 힌트 없이 다시 도전하면 받을 수 있어요.' : review ? '복습을 완료했어요.' : step === 5 && onComplete ? '기초 학습 완료. 이제 이야기 속 자료에 적용해봐요.' : step === 5 ? '도수분포표 1차 학습 완료. 다음은 복습으로 확인해요.' : '다음 단계로 진행하세요.'}</p></div>}
            </div>
      ...
      <footer className="fl-local-footer"><span>{onComplete ? '학습 진행은 이 기기에 임시 저장' : '운영 DB 연결 없음 · 브라우저 저장 허용 시 이 기기에만 기록'}</span>{resetConfirm ? <div>체험 기록을 초기화할까요? <button onClick={() => { setProgress(EMPTY_PROGRESS); goTo(0); setResetConfirm(false); }}>초기화</button><button onClick={() => setResetConfirm(false)}>취소</button></div> : <button onClick={() => setResetConfirm(true)}>체험 초기화</button>}</footer>
    </main>
  </div>;
}
