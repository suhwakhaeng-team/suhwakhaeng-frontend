...
function StoryExperience({ onComplete }: { onComplete?: () => void }) {
  ...
  return <div className="fs-shell">
    ...
    <main className="fs-main">
      ...
      {finished ? <section className="fs-scene fs-finish"><span className="fs-finish-star">✦</span><p className="fs-eyebrow">체험 완료 · 최초 완료 보상 100 XP</p><h2>도수분포표 학습 완료</h2><div className="fs-poster"><h3>우리 반 통학 시간</h3>{RANGES.map((range,index) => <div className="fs-poster-row" key={range}><span>{range}</span><i style={{ width: `${counts[index] / STORY_DATA.length * 100}%` }} /><strong>{counts[index]}명</strong></div>)}<p>가장 많은 구간: 20 이상 30 미만<br />30분 이상 걸리는 친구: 2명</p></div><div className="fs-concept-earned"><span>✓</span><div><strong>도수분포표 · 이번 체험 완료</strong><small>{onComplete ? '이제 실전 문제로 확인해요.' : '운영 숙련도에는 반영되지 않아요.'}</small></div></div>{onComplete && <button className="fs-primary" onClick={onComplete}>실전 문제 풀기 →</button>}<button className={onComplete ? 'fs-secondary' : 'fs-primary'} onClick={() => { setChapter(0); setGuess(null); setPlaced({}); setCard(null); setSeenTerms([]); setTerm('range'); setAnswers([]); setFinished(false); setMessage(''); setReportReady(false); }}>처음부터 다시 해보기</button></section>
