...
function StoryExperience({ onComplete }: { onComplete?: () => void }) {
  ...
  return <div className="fs-shell">
    ...
    <main className="fs-main">
      ...
      <footer className="fs-local"><span>{onComplete ? '가상 학습 자료 · 진행은 이 기기에 임시 저장' : '버전 2 · 가상 예시 · 운영 DB 연결 없음'}</span>{resetConfirm ? <span>상황 적용 완료 기록을 초기화할까요? <button onClick={() => { setEverCompleted(false); setResetConfirm(false); }}>기록 초기화</button><button onClick={() => setResetConfirm(false)}>취소</button></span> : <button onClick={() => setResetConfirm(true)}>진행 기록 초기화</button>}</footer>
    </main>
  </div>;
}
